...
# Iterate through all paths in sys.path and print them
sys.path.append('/home/matteo_sammut/4D-convnet-conda-test/4D-convnet/code')
...
parser.add_argument("-c", "--config",
                    help="path to config file", default='code/config/config.yaml')
...
class Indoor3DSemSeg(data.Dataset):
    def __init__(self, cfg,
        ...
        self.data_dir = pathlib.Path(cfg["data"]["path"])


        self.aug = aug

        self.train, self.num_points = train, cfg["data"]["num_points"]
...
if __name__ == "__main__":
    ...
    dset = Indoor3DSemSeg(cfg, train=True)
    print(dset[0])
    print(len(dset))
